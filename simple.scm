...
(define whoops
  (let ((string? string?) (string–append string–append)
	(error error)
...
(define random
  (let ((*seed* 1) (quotient quotient) (modulo modulo) (+ +) (– –) (* *) (> >))
	...
	(if (> test 0)
      ...
      (modulo *seed* x))))
;;; Logo–style word/sentence implementation(define word?
(let ((number? number?) (symbol? symbol?) (string? string?))
  (lambda (x)
    (or (symbol? x) (number? x) (string? x)))))
(define sentence?
(let ((null? null?) (pair? pair?) (word? word?) (car car) (cdr cdr))
  (define (list–of–words? 1)
    (cond ((null? 1) #t)
	  ((pair? 1)
	   (and (word? (car 1)) (list–of–words? (cdr 1))))
	  (else #f)))
  list–of–words?))
(define empty?
...
(lambda (string)
  (define (subsequents? string i length)
    (cond ((= i length) #t)
	  ...
	  (else #f)))
  (define (special–id? string)(or (string=? string "+")
				  (string=? string "–")
				  (string=? string " . . . ")))
    ...
    (cond ((= i len)
	  ...
	  ((char=? #\. (string–ref string i))
	   (cond (seen–point? #f)
		 ...
		 (else (nn–helper string (+ i l) len #t))))
...
(let ((= =) (char–rank char–rank) (make–string make–string) (char=? char=?)
  ...
  (lambda (char)
    (let ((rank (char–rank char))
      ...
      (cond ((= rank 0) (string–>symbol string))((= rank 2) (string–>number string))
	    ((char=? char #\+) '+)
	    ((char=? char #\–) '–)
	    (else string))))))
(define word–>string
  ...
  (let ((read–char read–char) (eqv? eqv?) (apply apply)
	...
	(cond ((or (eof–object? char) (eqv? char #\newline))
	       (apply string–append
		      (reverse
		       (cons
			(substring (car chars) 0 chunk–length)(cdr chars)))))
	      ((>= chunk–length 80)
...
(define strings–are–numbers
  (let ((are–they? #f)
	...
	(real–1cm 1cm) (real–list–ref list–ref) (real–log log)
    ...
    (lambda (yesno)
      (cond ((and are–they? (eq? yesno #t))
	    ...
	    ((eq? yesno #t)
	     ...
	     (set!
	      (set!
	       (set!
		(set!
		 tan (logoize–1 real–tan))
		truncate (logoize–1 real–truncate))
	       zero? (logoize–1 real–zero?))
	      vector–ref
	    ...
	    ((not yesno)
	     ...
	     (set! item real–item)
	     (set! 1cm real–1cm) (set! list–ref real–list–ref)
